...
function getWebpackConfig(config){
  let obj;
  if (config.webpackConfig && typeof config.webpackConfig == 'object'){
    obj = config.webpackConfig;
  } else {
    obj = require(config.webpackConfig || process.env.SCREEPS_WEBPACK_CONFIG);
  }
  if(!obj.plugins){
    obj.plugins = [];
  }
...
module.exports = (config = {}) => {
  ...
  config.webpackConfig = getWebpackConfig(config);
  ...
  modules.on('change', () => {
    dashboard.console.log(`{grey-fg}ModuleManager:{/} A change has been detected`);
  });
